...
const CodeCrafterApp = {
  ...
  setupScrollAnimations() {
    ...
    // Observe elements
    document
      .querySelectorAll("section, .note-card, .course-card")
      .forEach((el) => {
        observer.observe(el);
      });
  },
  ...
  setupFormHandling() {
    ...
    const contactForm = document.querySelector(".contact-form");
    if (contactForm) {
      // Formspree को naturally submit करेगा
      contactForm.addEventListener("submit", (e) => {
  ...
  setupCardAnimations() {
    document
      .querySelectorAll(".note-card, .course-card")
      .forEach((card, index) => {
    ...
    document
      .querySelectorAll(".cta-button, .card-button, .submit-btn")
      .forEach((btn) => {
